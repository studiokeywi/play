...
class Confetto {
  ...
  #turnSpeed;
  #velocity;
  constructor({ canvas, colors, ctx, drag, gravity, termVelocity }) {
    this.#canvas = canvas;
  ...
  draw(idx, confetti) {
    ...
    this.#updateVelocity();
    this.#updateLocation();
    if (!this.#handleOffscreen(idx, confetti))
      this.#render();
  }
  #handleOffscreen(idx, confetti) {
...
export default ({
  ...
  const confetti = [];
  const confettoCfg = { canvas, colors, ctx, drag, gravity, termVelocity };
  let frameRequest;
  const init = () => {
    resize();
    ...
    if (holidays)
      colors = holidayColors() ?? colors;
    if (!confetti.length)
      return void confetti.push(...[...Array(count)].map(() => new Confetto(confettoCfg)));
  ...
  const render = () => {
    ...
    if (confetti.length)
      return void (frameRequest = requestAnimationFrame(render));
    cancelAnimationFrame(frameRequest);
    canvas.remove();
